...
class DisplayManager:
    ...
    def _setup_matrix(self):
        ...
        try:
            ...
            logger.info(f"Config values from hardware_config: chain_length={hardware_config.get('chain_length')}, cols={hardware_config.get('cols')}, rows={hardware_config.get('rows')}")
            
            # Initialize the matrix
            self.matrix = RGBMatrix(options=options)
            logger.info("RGB Matrix initialized successfully")
            logger.info(f"Matrix actual dimensions after init: width={self.matrix.width}, height={self.matrix.height}")
            ...
            # Warn if dimensions don't match expected
            expected_width = options.cols * options.chain_length
            if self.matrix.width != expected_width:
                logger.warning(f"Matrix width mismatch! Expected {expected_width} (cols={options.cols} * chain={options.chain_length}), but matrix reports {self.matrix.width}")
