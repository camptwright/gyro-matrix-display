...
class StockManager:
    ...
    def display_stocks(self, force_clear: bool = False):
        ...
        # Ensure current_stock_index is valid
        if self.current_stock_index >= len(symbols):
            self.current_stock_index = 0
        
        # Get the current stock to display (no scrolling, just show one at a time)
        ...
        # Create display for current stock only (no scrolling)
        if self.cached_text_image is None or force_clear:
            is_crypto = current_data.get('is_crypto', False)
            
            # Create stock display for current symbol only
            stock_image = self._create_stock_display(
                current_symbol, 
                current_data['price'], 
                current_data['change'], 
                current_data['change'] / current_data['open'] * 100,
                is_crypto
            )
            
            # Crop to display width (in case it's wider)
            if stock_image.width > width:
                stock_image = stock_image.crop((0, 0, width, height))
            
            # Cache the image
            self.cached_text_image = stock_image
            self.last_update = time.time()
        
        # Disable auto-scrolling - static display only
        self.display_manager.set_scrolling_state(False)
        
        # Process any deferred updates
        self.display_manager.process_deferred_updates()
        
        # Display the current stock (no scrolling)
