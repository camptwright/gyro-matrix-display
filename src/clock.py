...
class Clock:
    ...
    def display_time(self, force_clear: bool = False) -> None:
        ...
        if time_str != self.last_time or date_str != self.last_date or force_clear:
            ...
            # Draw time (centered, near top) - use smaller font to fit AM/PM
            time_width = self.display_manager.small_font.getlength(time_str)
            ...
            # Draw AM/PM (small, to the right of time, but ensure it fits)
            ampm_width = self.display_manager.small_font.getlength(ampm)
            ampm_x = time_x + time_width + 2  # Small gap after time
            # Make sure AM/PM doesn't go off screen
            if ampm_x + ampm_width > display_width:
            ...
            # Draw weekday on first line (small font, ensure it fits)
            weekday_width = self.display_manager.small_font.getlength(weekday)
            weekday_x = max(0, (display_width - weekday_width) // 2)  # Center but don't go negative
            self.display_manager.draw_text(
                weekday,
                x=weekday_x,
            ...
            # Draw month and day on second line (small font, ensure it fits)
            date_width = self.display_manager.small_font.getlength(date_str)
            date_x = max(0, (display_width - date_width) // 2)  # Center but don't go negative
            self.display_manager.draw_text(
                date_str,
                x=date_x,
