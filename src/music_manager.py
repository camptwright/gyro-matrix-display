...
class MusicManager:
    ...
    def display(self, force_clear: bool = False):
        ...
        # Make album art smaller (24px instead of full height)
        album_art_size = 24
        album_art_target_size = (album_art_size, album_art_size)
        album_art_x = 0
        album_art_y = (matrix_height - album_art_size) // 2  # Center vertically
        text_area_x_start = album_art_x + album_art_size + 3  # Add more spacing
        ...
        # Use extra_small_font for smaller text that won't overlap
        font_title = self.display_manager.extra_small_font
        ...
        try:
            ...
            line_height_title = 7  # Fallback for small fonts
        
        # Use a static value for small font line height
        LINE_HEIGHT_SMALL = 7  # Fixed pixel height for small font
        PADDING_BETWEEN_LINES = 2  # Increased padding to prevent overlap

        # Calculate y positions - start from top with proper spacing
        matrix_height = self.display_manager.matrix.height
        
        # Position text starting from top, ensuring no overlap with album art
        y_pos_title_top = 2  # Start a bit from top
        y_pos_artist_top = y_pos_title_top + line_height_title + PADDING_BETWEEN_LINES
        y_pos_album_top = y_pos_artist_top + LINE_HEIGHT_SMALL + PADDING_BETWEEN_LINES
