...
class DisplayController:
    def __init__(self):
        ...
        self.clock = Clock(self.display_manager, self.config) if self.config.get('clock', {}).get('enabled', True) else None
            logger.info(f"Clock manager: {'Initialized' if self.clock else 'Disabled'}")
        ...
        self.weather = WeatherManager(self.config, self.display_manager) if self.config.get('weather', {}).get('enabled', False) else None
            logger.info(f"Weather manager: {'Initialized' if self.weather else 'Disabled'}")
        except Exception as e:
            logger.error(f"Failed to initialize WeatherManager: {e}", exc_info=True)
            self.weather = None
        
        try:
        self.stocks = StockManager(self.config, self.display_manager) if self.config.get('stocks', {}).get('enabled', False) else None
            logger.info(f"Stocks manager: {'Initialized' if self.stocks else 'Disabled'}")
        except Exception as e:
            logger.error(f"Failed to initialize StockManager: {e}", exc_info=True)
            self.stocks = None
        ...
        if self.clock: 
            self.available_modes.append('clock')
            logger.debug("Added 'clock' to available_modes")
        if self.weather: 
            self.available_modes.extend(['weather_current', 'weather_hourly', 'weather_daily'])
            logger.debug("Added weather modes to available_modes")
        else:
            logger.warning(f"Weather manager is None - config says enabled={self.config.get('weather', {}).get('enabled', False)}")
        if self.stocks: 
            self.available_modes.append('stocks')
            logger.debug("Added 'stocks' to available_modes")
        else:
            logger.warning(f"Stocks manager is None - config says enabled={self.config.get('stocks', {}).get('enabled', False)}")
    ...
    def run(self):
        ...
        try:
            # Display clock immediately while other data loads in background
            self.current_display_mode = self.available_modes[self.current_mode_index] if self.available_modes else 'none'
            self.last_switch = time.time()  # Initialize last_switch for immediate display
            self.force_clear = True  # Force clear on first display
            
            # Display clock immediately if it's the first mode
            if self.current_display_mode == 'clock' and self.clock:
                logger.info("Displaying clock immediately while other data loads in background...")
                try:
                    self.clock.display_time(force_clear=True)
                    logger.info("Clock displayed successfully")
                except Exception as e:
                    logger.error(f"Error displaying initial clock: {e}", exc_info=True)
            
            # Clear cache and fetch data in background (non-blocking)
            import threading
            def background_init():
            logger.info("Clearing cache and refetching data to prevent stale data issues...")
            self.cache_manager.clear_cache()
            self._update_modules()
                logger.info("Background data fetch completed")
            
            init_thread = threading.Thread(target=background_init, daemon=True, name="background-init")
            init_thread.start()
            
            # Small delay to ensure clock is displayed before entering main loop
            time.sleep(0.1)
            
            while True:
                current_time = time.time()
                
                # Check if mode was changed externally (e.g., by display_adapter)
                # Sync current_display_mode with current_mode_index if they don't match
                expected_mode = self.available_modes[self.current_mode_index] if self.available_modes and self.current_mode_index < len(self.available_modes) else 'none'
                mode_changed_externally = False
                if self.current_display_mode != expected_mode:
                    logger.info(f"Mode changed externally: {self.current_display_mode} -> {expected_mode} (index: {self.current_mode_index})")
                    self.current_display_mode = expected_mode
                    self.force_clear = True
                    self.last_switch = current_time  # Reset timer for new mode
                    ...
                    if live_priority_takeover:
                        ...
                        if self.current_display_mode.endswith('_live'):
                            ...
                            if current_time - self.last_switch >= self.get_current_duration():
                                ...
                                if hasattr(self, '_last_logged_duration'):
                                    delattr(self, '_last_logged_duration')
                        elif current_time - self.last_switch >= self.get_current_duration() or self.force_change or mode_changed_externally:
                            self.force_change = False
